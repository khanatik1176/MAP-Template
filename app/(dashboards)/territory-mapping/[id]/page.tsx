...
const UNIONS_BY_DISTRICT: Record<number, { id: number; name: string }[]> =
  DISTRICTS.reduce<Record<number, { id: number; name: string }[]>>((acc, d) => {
    // produce up to 60 unions for larger testing on some districts
...
/**
 * Compute unique unions for selected districts.
 */
...
/**
 * Custom Modal (portal) - simple, accessible.
 */
function Modal({
  open,
  onOpenChange,
  ...
  return ReactDOM.createPortal(
    ...
    >
      ...
      <div className="fixed inset-0 flex items-center justify-center p-4">
        <div
          className="w-full max-w-[800px] h-full max-h-[550px] transform overflow-hidden rounded-lg bg-white shadow-lg ring-1 ring-black/5"
...
/**
 * Lightweight searchable multiselect dropdown component
 * - options: {id, name}[]
 * - selected: number[]
 * - dropdownMaxHeight: pixel number for large lists
 */
function MultiSelect({
  ...
  return (
    <div className="relative text-left" ref={containerRef}>
      ...
      {open && (
        <div
          className="absolute z-40 my-2  w-full rounded-md border bg-white shadow-lg"
        ...
        >
          ...
          <div
            className="h-full max-h-[100px] overflow-auto p-2"
            role="listbox"
          >
            {filtered.length === 0 && (
            ...
            <Modal open={modalOpen} onOpenChange={setModalOpen} title="Create Territory">
              <div className="p-4">
                <div className="flex items-start justify-between">
                  ...
                  <div>
                    <button
                      ...
                      className="rounded bg-transparent px-2 py-1 text-sm"
                    >
                      ✕
                    </button>
                  </div>
                ...
                <div className="mt-4 space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mt-2">
                      <MultiSelect
                        ...
                        onChange={(next) => setSelectedDistrictIds(next)}
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-700">
                      Unions (based on selected districts)
                    </label>
                    <div className="mt-2">
                      <MultiSelect
                        options={availableUnions}
                        selected={selectedUnionIds}
                        onChange={(next) => setSelectedUnionIds(next)}
                        placeholder={
                          availableUnions.length === 0 ? "Select districts first" : "Select unions"
                        }
                        // allow larger scroll area for unions (many entries)
                        dropdownMaxHeight={360}
                      />
                    </div>
                    {availableUnions.length > 250 && (
                      <p className="mt-1 text-xs text-muted-foreground">
                        Large list — use search to find unions faster.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-36 flex justify-end gap-2">
                  ...
                  <Button variant="greenish" size="md" onClick={handleCreate}>
                    Create
                  </Button>
                </div>
              </div>
        ...
        {territories.length === 0 ? (
          <section className="flex min-h-[60vh] items-center justify-center">
            <div className="max-w-md text-center">
              ...
              <h2 className="mt-6 text-2xl font-semibold text-gray-900">No territory available</h2>

              <p className="mt-2 text-sm text-gray-500">
                You don't have any territories yet. Add territories to start mapping areas and assign them to users or teams.
              </p>
            </div>
          </section>
        ) : (
